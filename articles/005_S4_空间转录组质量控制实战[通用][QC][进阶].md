...
**这个对比能说明什么**：
- 组织外 spot 的 UMI 数显著低于组织内，验证了图像分割的准确性
- 但需要注意：并非所有组织内 spot 都是高质量的，仍需进一步过滤
...
Visium HD (High Definition, 高分辨率) 是 10x Genomics 推出的高分辨率空间转录组平台，提供 2 µm 和 8 µm 两种分辨率，QC 策略需要调整：
...
**这个分析能说明什么**：
- 可以揭示不同样本间的技术变异程度
- 但需要注意：这些差异是否需要批次校正，需要结合生物学背景判断
...
本文建立了空间转录组质量控制的完整方法论，核心要点包括：

**QC 不是简单的阈值过滤，而是数据驱动的决策过程**。通过计算三大核心指标（UMI 数、基因数、线粒体比例），我们能够量化每个 spot 的质量。但关键在于：阈值不是固定的，而是基于数据分布的百分位数动态确定。这种方法避免了"一刀切"的问题，能够适应不同组织类型和测序深度的差异。

**空间可视化揭示了分布图无法发现的问题**。将 QC 指标映射到组织切片上，能够识别边缘效应、局部坏死区域、技术伪影等空间模式。均匀分布说明技术质量良好，而空间聚集的低质量 spot 可能反映真实的生物学状态（如肿瘤坏死区），需要结合组织学知识判断是否过滤。

**负对照验证是阈值合理性的关键证据**。组织外 spot 作为天然的负对照，其 UMI 数应该显著低于组织内（通常 10-100 倍差异）。Mann-Whitney U 检验提供了统计显著性，但更重要的是：如果组织内外差异不明显，说明存在污染或图像分割失败，必须重新评估数据质量。

**敏感性分析评估过滤策略的稳健性**。通过测试宽松、适中、严格三种阈值场景，我们能够量化过滤对数据保留率和质量指标的影响。Moderate 策略通常是最佳选择：保留 95% 以上的组织内 spot，同时确保中位数指标提升。过度过滤（Stringent）可能丢失边缘区域的真实信号，而过于宽松（Lenient）则保留了低质量数据。

**特殊场景需要调整 QC 策略**。Visium HD 数据的 bin 更小，UMI 数更低，阈值需要相应降低。成像型平台（Xenium、CosMx）关注解码质量和细胞分割，而非 UMI 数。多样本分析需要检查批次效应，判断是否需要批次校正。组织类型也影响阈值：心肌、肝脏的线粒体比例天然较高，不能简单套用脑组织的标准。

**常见错误及其修复方法**。过度过滤导致空间结构丢失，解决方法是分区域设置阈值或使用空间平滑后的指标。线粒体阈值设置不当，需要参考组织特异性标准。忽略空间连续性，导致"孤岛"或"空洞"，需要结合邻域信息过滤。基因过滤过于激进，可能丢失低表达但重要的转录因子，需要动态调整阈值或保留已知标志基因。

**最终交付的不仅是清理后的数据，更是完整的质控证据链**。包括诊断图（分布图、空间图、前后对比图）、统计检验结果、敏感性分析报告、过滤参数记录。这些证据确保分析的可复现性和可信度，也为审稿人和读者提供了透明的质量评估依据。
